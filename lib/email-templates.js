...
// Style : aligne sur la marque (rouge #E4032C / #C41230, typo Schibsted Grotesk + Manrope)
...
// Compose de :
//   1) Une bande ticker fine (defilante) avec toutes les infos clefs sur UNE seule ligne
//   2) Le bloc logo principal
// Compatible clients mail : <marquee> + @keyframes en fallback, contenu duplique pour defilement infini sans coupure
function renderHeader({ logoUrl, badge = '', marqueeItems = [] } = {}){
  const items = marqueeItems && marqueeItems.length
    ? marqueeItems
    : [
        badge || 'NISSI ASSURANCES',
        'Abidjan - Cote d\'Ivoire',
        'Courtier agree',
        'Ouvert 24/7',
        '+225 27 22 00 00 00',
        'contact@nissi-assurances.ci',
        'Votre protection, notre priorite'
      ];
  // On duplique pour une boucle seamless (sans "trou" au passage)
  const seq = items.concat(items);

  return `
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${BRAND.red};">
    <tr>
      <td style="padding:0;">

        <!-- 1) TICKER defilant (une seule ligne, jamais de retour a la ligne) -->
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:linear-gradient(90deg,#a10f25,#C41230 50%,#a10f25);border-bottom:1px solid rgba(255,255,255,.22)">
          <tr>
            <td align="left" valign="middle" style="padding:0;overflow:hidden;color:#fff;font:700 11.5px/1 'Manrope',Helvetica,Arial,sans-serif;letter-spacing:.12em;text-transform:uppercase;white-space:nowrap">
              <style>
                @keyframes nissi-ticker {
                ...
                .nissi-ticker-track {
                  ...
                  white-space:nowrap;
                  animation: nissi-ticker 38s linear infinite;
                ...
                @media (prefers-reduced-motion: reduce) {
                  .nissi-ticker-track { animation: none; transform: translateX(0); }
                }
              </style>
              <div class="nissi-ticker-wrap" style="overflow:hidden;width:100%;padding:11px 0">
                <span class="nissi-ticker-track">
                  ${seq.map((t, i) => `
                    <span class="nissi-ticker-item">${esc(t)}</span>${i < seq.length - 1 ? '<span class="nissi-ticker-dot"></span>' : ''}
                  `).join('')}
                </span>
                <!-- Fallback sans animation (clients mail qui strippent CSS) -->
                <span style="display:none;white-space:nowrap">${items.map((t, i) => `${esc(t)}${i < items.length - 1 ? ' &middot; ' : ''}`).join('')}</span>
              </div>
              <!-- <marquee> en filet de securite (Apple Mail / iOS Mail) -->
              <marquee behavior="scroll" direction="left" scrollamount="2" style="display:none;width:100%">
                ${items.map((t, i) => `${esc(t)}${i < items.length - 1 ? ' &middot; ' : ''}`).join('')}
              </marquee>
            </td>
          </tr>
        </table>

        <!-- 2) BLOC LOGO principal (jamais de 2e ligne : badge integre au ticker) -->
      ...
      <tr>
        <td align="center" style="padding:40px 20px 60px;">
          ...
          <p style="margin:24px 0 0;font:400 12px 'Manrope',Helvetica,Arial,sans-serif;color:${BRAND.mute};text-align:center">
            Cette notification a été générée automatiquement par le formulaire de nissi-assurances.ci
          </p>
        </td>
      </tr>
    </table>
  ...
  const content = `
    ${renderHeader({
      ...
      marqueeItems: [
        ...
        'Bonjour ' + firstName,
        'Reponse sous 24h ouvrees',
        'NISSI Assurances - Cote d\'Ivoire',
        'Votre protection, notre priorite',
        '+225 27 22 00 00 00'
      ]
    })}
